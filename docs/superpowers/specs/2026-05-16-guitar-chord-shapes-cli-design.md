...
- File naming: `{root}{chord_type}.txt`
  ...
  - Slash chords: `{root}.{bass}.txt` and `{root}m.{bass}.txt` for all 11 bass notes
...
```
URL generation → fetcher.py → cache (~/.cache/smallcaged/)
                        ↓
...
```
smallcaged [--chord CHORD] [--root ROOT] [--output FILE] [--refresh]
```
